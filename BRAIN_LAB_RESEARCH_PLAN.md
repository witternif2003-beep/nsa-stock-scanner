...
| **ERR-11** | `backend/brain_lab.py` | **Cross-Sectional NaN Contagion in PAMR-E**: Passing non-finite numbers (`NaN`, `Inf`, or `None`) in quote metrics corrupted the sample mean and variance in `calc_z()`, propagating `NaN` across the entire universe ranking. | Implemented `safe_float()` filter sanitizing non-finite values into bounded numeric defaults prior to cross-sectional standardization. | **PASS (Zero NaN Propagation)** |
| **ERR-12** | `backend/brain_lab.py` | **Comma-Separated Number String Parsing (`ValueError`)**: Formatted strings like `'1,486.97x'` failed with `ValueError: could not convert string to float: '1,486.97'` due to comma thousands-separators. | Added comprehensive regex stripping for commas (`,`), dollar signs (`$`), percentages (`%`), and unit markers (`x`, `—`). | **PASS (Robust String Sanitization)** |
| **ERR-13** | `backend/server.py` | **Async Await on Synchronous Yahoo Quote Function**: In `post_brain_lab_research`, invoking `await fetch_yahoo_v8_quote(sym)` threw `TypeError: object dict can't be used in 'await' expression` because `fetch_yahoo_v8_quote` is synchronous. | Removed invalid `await` keyword, calling `fetch_yahoo_v8_quote(sym)` directly with null-safe parameter unpacking. | **PASS (Zero Runtime Exception)** |
| **ERR-14** | `backend/server.py` | **Missing Serverless Route Prefixes for POST Endpoints**: Endpoints `/api/brain-lab/validate`, `/api/brain-lab/performance-match`, and `/api/brain-lab/research` lacked subpath `@app.post("/brain-lab/...")` decorators required by Vercel request rewriting. | Dual-decorated all POST endpoints with both `/api/brain-lab/...` and `/brain-lab/...` to guarantee seamless serverless dispatch. | **PASS (100% Serverless Route Parity)** |
| **ERR-15** | `public/serenity-widget.html` | **Client-Side Float Parsing and Zero Variance Protection**: In `getZ()`, when array variance was zero or input was non-finite, division by zero generated `NaN` in client card conviction scores. | Built `safeNum()` helper and clamped standard deviation to `safeStd = isFinite(std) && std > 1e-7 ? std : 1.0`. | **PASS (Zero UI Render Glitches)** |
